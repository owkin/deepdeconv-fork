...
Deconvolution <- function(T,C){

    T <- T[rownames(T) %in% rownames(C),]
    C <- C[rownames(C) %in% rownames(T),]
    T <- T[order(match(rownames(T),rownames(C))),]
    stopifnot(identical(rownames(T),rownames(C)))

    out <- list()
    methods <- c("DeconRNASeq","wnnls","OLS","RLR","DWLS_OLS","DWLS_WLS","DWLS_SVR","FARDEEP","nnls","nnls_mind")

    # DeconRNAseq
    out[[1]] = t(DeconRNASeq::DeconRNASeq(datasets = as.data.frame(T), signatures = as.data.frame(C), proportions = NULL, checksig = FALSE, known.prop = FALSE, use.scale = FALSE, fig = FALSE)$out.all)
    colnames(out[[1]]) = colnames(T)

    # wnnls
    wnnls <- deconv_simple(as.matrix(T), as.matrix(C), iter.max = 2000, nu = 1e-10, epsilon = 0.001, truep = NULL)
    out[[2]] <- t(wnnls$prop.est.mvw)

    out[[2]] = do.call(cbind.data.frame,lapply(apply(T,2,function(x) nnls::nnls(as.matrix(C),x)), function(y) y$x))
    out[[2]] = apply(out[[2]],2,function(x) x/sum(x)) #explicit STO constraint
    rownames(out[[2]]) <- colnames(C)

    # OLS
    out[[3]] = apply(T,2,function(x) lm(x ~ as.matrix(C))$coefficients[-1])
    out[[3]] = apply(out[[3]],2,function(x) ifelse(x < 0, 0, x)) #explicit non-negativity constraint
    out[[3]] = apply(out[[3]],2,function(x) x/sum(x)) #explicit STO constraint
    rownames(out[[3]]) <- unlist(lapply(strsplit(rownames(out[[3]]),")"),function(x) x[2]))

    # RLR
    require(MASS)
    out[[4]] = do.call(cbind.data.frame,lapply(apply(T,2,function(x) MASS::rlm(x ~ as.matrix(C), maxit=100)), function(y) y$coefficients[-1]))
    out[[4]] = apply(out[[4]],2,function(x) ifelse(x < 0, 0, x)) #explicit non-negativity constraint
    out[[4]] = apply(out[[4]],2,function(x) x/sum(x)) #explicit STO constraint
    rownames(out[[4]]) <- unlist(lapply(strsplit(rownames(out[[4]]),")"),function(x) x[2]))

    # DWLS
    solutionsOLS<-c()
    solutionsDWLSIterative<-c()
    solutionsSVR<-c()
    for (i in 1:ncol(T)){
      m_j<-T[,i]
      B_j<- as.matrix(C)

      sol <- try(solveOLS(B_j,m_j))
      if(class(sol) == "try-error"){sol <- rep(NA,ncol(C))} else{sol<-round(sol,5)}

      solDWLSIterative <- try(solveDampenedWLS(B_j,m_j))
      if(class(solDWLSIterative) == "try-error"){solDWLSIterative <- rep(NA,ncol(C))} else{solDWLSIterative<-round(solDWLSIterative,5)}

      solSVR <- try(solveSVR(B_j,m_j))
      if(class(solSVR) == "try-error"){solSVR <- rep(NA,ncol(C))} else{solSVR<-round(solSVR,5)}

      solutionsOLS<-cbind(solutionsOLS,sol)
      solutionsDWLSIterative<-cbind(solutionsDWLSIterative,solDWLSIterative)
      solutionsSVR<-cbind(solutionsSVR,solSVR)
    }
    colnames(solutionsOLS) <- colnames(T)
    solutionsOLS <- solutionsOLS[,colSums(is.na(solutionsOLS)) < nrow(solutionsOLS)]
    colnames(solutionsDWLSIterative) <- colnames(T)
    solutionsDWLSIterative <- solutionsDWLSIterative[,colSums(is.na(solutionsDWLSIterative)) < nrow(solutionsDWLSIterative)]
    colnames(solutionsSVR) <- colnames(T)
    solutionsSVR <- solutionsSVR[,colSums(is.na(solutionsSVR)) < nrow(solutionsSVR)]

    out[[5]] <- solutionsOLS
    out[[6]] <- solutionsDWLSIterative
    out[[7]] <- solutionsSVR

    require(FARDEEP)
    FD = t(FARDEEP::fardeep(C, T, nn = TRUE, intercept = TRUE, permn = 10, QN = FALSE)$abs.beta)
    FD = apply(FD,2,function(x) x/sum(x)) #explicit STO constraint
    out[[8]] <- FD

    # require(ADAPTS)
    # nnls <- estCellPercent.nnls(C, T)


    nnls = do.call(cbind.data.frame,lapply(apply(as.matrix(T),2,function(x) nnls::nnls(as.matrix(C),x)), function(y) y$x))
    nnls = apply(nnls,2,function(x) x/sum(x))
    rownames(nnls) <- colnames(C)
    out[[9]] <- nnls

    nnls_mind = t(est_frac(C,T))
    rownames(nnls_mind) <- colnames(C)
    out[[10]] <- nnls_mind
